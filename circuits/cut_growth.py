...
def calculate_min_cut(n_qubits, depth, circuits_trial, exclude_nodes, cut_trials, dat_file_path_circuits, bool_entropy=None, alpha=None):
    ...
    if bool_entropy is not None and bool_entropy==True:
...
def process_trials_entropy(n_qubits, depth, exclude_nodes, circuits_trials, cut_trials, dat_file_path_circuits, alpha):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=24) as executor:
        # Prepare a task list for trials
        futures = [executor.submit(calculate_min_cut, n_qubits, depth, trial, exclude_nodes, cut_trials, dat_file_path_circuits, bool_entropy=True, alpha=alpha) for trial in range(circuits_trials)]
...
def cut_growth(total_qubits, total_depth, exclude_nodes, circuits_trials, cut_trials, fixed=None, bool_entropy=None, alpha=None):
    
    if bool_entropy is None:
        bool_entropy = False
...
def histogram_min_cut(total_qubits, total_depth, exclude_nodes, circuits_trials, cut_trials):
    ...
    with open(dat_file_path_results, 'w') as dat_file:
        ...
        for min_cut_value in min_cut_values:
